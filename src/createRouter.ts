...
export const bindRouter = <
  Router extends {
    navigate: (url: string, opts?: NavigationOptions) => void;
  },
...
>(
  ...
  for (const pattern in routes) {
    if (isRoutePattern(pattern)) {
      ...
      path.navigate = (params, opts) => {
        const target = path(params as unknown as Parameters<typeof path>[0]);
        router.navigate(
          path(params as unknown as Parameters<typeof path>[0]).slice(
            basename.length
          ),
          opts
        );
...
export type DOMRouterOpts<Basename extends Pattern> = Parameters<
...
export const createRouter = <
  ...
  Basename extends Pattern = "/"
>(
  routeObject: Array<T>,
  options: DOMRouterOpts<Basename> & {
  ...
  const routes = inferRouteObject(
    ...
    opts?.basename
  ) as unknown as InferRoute<{ children: [T] }, Basename>;
  return Object.assign(router, bindRouter(router, routes, opts?.basename));
};
